...
class wsScreenOptions10 {
	...
	function add_autosave_script(){
		...
		if ( $got_autosave ){
			...
			$url = plugins_url('linksynceparcel').'/assets/js/screen-options.js';
...
// Do action
function eparcel_default_settings_panel(){
    $defaults = eparcel_get_default_settings();
    ...
    foreach($fields as $field => $legend){
        ...
        $output .= sprintf(
            '<label for="rawhtml_default-%s" style="line-height: 20px;">
                <input type="checkbox" data-key="%s" name="rawhtml_default-%s" id="rawhtml_default-%s"%s>
                %s
            </label>',
            $esc_field,
            $esc_field,
	...
	// $output .= '<br><button type="button" id="eparcel-settings" class="button">Save Options</button>';
    return $output;
}

function eparcel_get_default_settings(){
 ...
 */
function eparcel_set_default_settings($new_defaults){
 ...
 * @return void
 */
function eparcel_save_new_defaults($params){
	...
	eparcel_set_default_settings($defaults);
}